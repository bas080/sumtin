...
function isEquallyDistributed(genFn, n = 1000000, similarity = 0.1) {
  const groups = {}
  ...
  function compare(groups) {
    ...
    if (delta(current / next, 1) > similarity)
      throw new Error(`${genFn.name} not equally distributed (${current} / ${next})`)
